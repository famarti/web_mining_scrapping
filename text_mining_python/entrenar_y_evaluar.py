...
# el clasificador que vamos a usar, NO tiene por que ser este, use lo que quiera probar.
clasificador = SVC(kernel='linear', probability=True)

# Si no usamos embeddings: cantidad maxima de features que seleccionara el extractor de features
MAX_FEATURES=150
...
# Comenzar a entrenar ye valuar usando cross-validation
n_fold = 1
accuracy_promedio = 0
...
for train_index, test_index in StratifiedKFold(n_splits=CANT_FOLDS_CV, random_state=None, shuffle=True).split(vectores, targets):
        ...
        # PASO 1: Seleccionar features a partir de los folds de entrenamiento
        #  OJO! Estos pasos asumen que c/columna es 1 token, NO Embeddings.
        ...
        # PASO 2; clasificar el fold de test
